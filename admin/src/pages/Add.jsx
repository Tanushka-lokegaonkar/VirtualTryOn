...
const Add = ({ token }) => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL || 'http://localhost:4000'}/api/product/add`,
        formData,
        { headers: { token } }
      );
  ...
  return (
    <form onSubmit={onSubmitHandler} className="flex flex-col w-full items-start gap-3">
        ...
        <div className="flex gap-3">
          {['S', 'M', 'L', 'XL', 'XXL'].map((size) => (
            <div key={size} onClick={() => toggleSize(size)}>
              <p
                className={`${
                  sizes.includes(size) ? 'bg-pink-100 border-pink-400' : 'bg-slate-200 border-transparent'
                } border px-3 py-1 cursor-pointer select-none`}
              >
                {size}
              </p>
            </div>
      ...
      <button type="submit" className="w-28 py-3 mt-4 bg-black text-white active:bg-gray-700">
        ADD
      </button>
    </form>
  );
